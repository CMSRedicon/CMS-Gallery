namespace Slug {
    ...
    export class Main {
        ...
        public ascii(value: any) {
            _.forEach(this.charsArray(), (val, key) => {
                value = this.replaceArray(value, val, key);
            });

            return value.replace(/[^\x20-\x7E]/u, '');
        }

        public pregQuote(str: any, delimiter?: any) {
            return (str + '').replace(new RegExp('[.\\\\+*?\\[\\^\\]$(){}=!<>|:\\' + (delimiter || '') + '-]', 'g'), '\\$&');
        }

        public replaceArray(string: any, find: any, replace: any) {
